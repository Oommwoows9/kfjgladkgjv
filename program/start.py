...
@Client.on_message(
    command(["/starhwywt", f"/statwywrt@{BOT_USERNAME}"]) & filters.private & ~filters.edited
)
...
""",
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        "🧠 ثقف عقلك", url="https://t.me/i170666"
                    )
                ],
            ]
        ),
...
@Client.on_message(
    command(["معلومات البوت","/alive", f"/alive@{BOT_USERNAME}"]) & filters.group & ~filters.edited
)
async def alive(client: Client, message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("🧠 ثقف عقلك", url=f"https://t.me/i170666"),
                InlineKeyboardButton(
                    "📣 قناة السورس", url=f"https://t.me/{UPDATES_CHANNEL}"
                ),
            ]
...
@Client.on_message(command(["سرعه البوت","/ping", f"/ping@{BOT_USERNAME}","سرعة البوت"]) & ~filters.edited)
...
@Client.on_message(command(["حاله البوت","/uptime", f"/uptime@{BOT_USERNAME}"]) & ~filters.edited)
